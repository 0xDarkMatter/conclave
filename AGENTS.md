...
13. **The claude CLI prints MCP diagnostics on STDOUT before its JSON envelope** (`Client.listTools() called but server does not advertise tools capability - returning empty list`, seen 2026-09-13). `claude.go` therefore *locates* the envelope (`parseClaudeJSONOutput`: first `{` that decodes as an object with a `result` key) instead of unmarshalling the whole buffer. Do not "simplify" it back to `json.Unmarshal(output)`; pinned by `TestClaudeCLIIgnoresLeadingNoiseBeforeJSON`.
