...
13. **The claude CLI prints MCP diagnostics on STDOUT before its JSON** (`Client.listTools() called but server does not advertise tools capability - returning empty list`, seen 2026-09-13), and `claude auth status` is pretty-printed multi-line JSON. Every reader of claude stdout (`Query`, `Preflight`, `SubscriptionLoggedIn`) therefore *locates* its object via `findJSONObject` in `claude.go` instead of unmarshalling the whole buffer; do not "simplify" any of them back to `json.Unmarshal(output)`. On an API error the CLI exits 1 but the only readable message is in the envelope's `result`, so `Query` uses `runCommandStdout` (stdout kept on failure) and surfaces it. Pinned by `TestClaudeCLIIgnoresLeadingNoiseBeforeJSON` and its siblings in `claude_cli_json_noise_test.go`.
