...
14. **claude runs isolated from the caller's cwd** (ADR-013): `--strict-mcp-config --setting-sources user --no-session-persistence`, in an empty temp directory, so a panel answer is not shaped by whichever repo conclave was invoked from (a bare "hi" once described the caller's worktree and cited its startup hook). Never swap this for `--bare`: it disables OAuth and breaks subscription auth (Gotcha 7). Context goes in via `-f`/stdin. Pinned by `TestClaudeCLIRunsIsolatedFromCallerContext`.
