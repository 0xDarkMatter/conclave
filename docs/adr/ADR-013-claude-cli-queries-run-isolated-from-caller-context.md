...
Every `claude --print` panel query runs with `--strict-mcp-config`, an empty `--setting-sources` list and `--no-session-persistence`, in an empty directory conclave owns rather than the caller's cwd, and never with `--bare`; context reaches the panel only through `-f` / stdin.
...
A third observation came from measuring `--setting-sources user` against the empty list on the same one-word prompt: user-level settings (instruction files, output style, rules) added about 52k prompt tokens to every query (56,855 vs 4,752 cache-creation tokens) and the answer arrived in the user's configured persona rather than a neutral voice. The empty list still authenticated over the subscription; only `--bare` drops OAuth.

Every provider in a judge panel must see the same prompt and nothing else. Neither the repo the user happens to run conclave from nor the user's own agent persona is part of the prompt.
...
- **`--setting-sources user` (keep user-level settings).** Tried first, on the theory that user settings travel with the user rather than the cwd. Rejected after measurement: it more than doubled per-query prompt tokens and coloured answers with the user's persona, which is exactly the contamination this record exists to remove.
...
- Roughly 52k fewer prompt tokens per claude query than with user settings loaded, and a neutral voice across seats.
...
- The user's own settings no longer reach the panel either: no user-scope hooks, output style or instruction files. A user who wants a persona on a panel seat must put it in the prompt.
